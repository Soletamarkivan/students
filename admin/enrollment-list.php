...
$query = "SELECT sn.student_number, s.surname, s.first_name, s.middle_name, s.suffix, ed.course_id, yl.year_level, cr.course_name, si.status, s.suffix, ed.admission_type, ed.reference_no
                            ...
                            <table class="table-auto w-full mt-4">
                                <thead>
                                    <tr class="px-2 py-1 bg-blue-300">
                                        ...
                                        <th class="text-center">Reference_no</th>
                                ...
                                <tbody id="studentTableBody">
                                    <?php foreach ($users as $index => $user) : ?>
                                        <tr class="px-2 py-1 <?= $index % 2 === 0 ? 'bg-white-500' : 'bg-blue-100' ?>">
                                            ...
                                            <td class="text-center"><?= isset($user['admission_type']) ? $user['admission_type'] : 'N/A'; ?></td>
                                            <td class="text-center"><?= isset($user['reference_no']) ? $user['reference_no'] : 'N/A'; ?></td>
                                            <td class="text-center"><?= isset($user['status']) ? $user['status'] : 'N/A'; ?></td>
                                            <td class="text-center">
                                                <?php if ($user['status'] === 'Enrolled') : ?>
                                                    <button type="button" onclick="enrollStudent(<?= $user['student_number']; ?>, <?= $index; ?>)" title="Mark as Official Enrolled">
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.success) {
                    ...
                    $('#statusCell_' + rowIndex).text('Official Enrolled');
