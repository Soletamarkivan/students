...
<body class="font-[roboto-serif]">
                    ...
                    <div class="flex justify-start gap-4">
                        ...
                        <div class="border-l-2 border-blue-300">
                            <div id="tabsContainer" class="pl-2 h-full">
                                ...
                                <div class="h-1/2">
                                    <div class="flex">
                                        <div id="requestsTab" class="tab px-4 py-2 cursor-pointer rounded-tl rounded-tr" onclick="showTab('requestsTabContent', 'requestsTab', 'notificationTab', 'receivedTab')">
                                            Requests
                                        </div>
                                        <div id="notificationTab" class="tab px-4 py-2 cursor-pointer rounded-tl rounded-tr" onclick="showTab('notificationTabContent', 'notificationTab', 'requestsTab', 'receivedTab')">
                                            Granted
                                        </div>
                                        <div id="receivedTab" class="tab px-4 py-2 cursor-pointer rounded-tl rounded-tr" onclick="showTab('receivedTabContent', 'receivedTab', 'requestsTab', 'notificationTab')">
                                            Received
                                        </div>
                                    </div>
    ...
    <?php if (!empty($requestMessages)) : ?>
        <?php foreach ($requestMessages as $requestMessage) : ?>
            <div class="border-t border-b border-gray-200 p-2 rounded request-item" style="width:300px;">
                ...
                <div class="text-xs text-gray-500">
                    <?php if (isset($requestMessage['relaese_date'])) : ?>
                        <?= formatDateHeading($requestMessage['release_datetime']); ?>
                    <?php else : ?>
                    <?php endif; ?>
                </div>
                <button name="grantbutton" class="px-4 py-2 cursor-pointer" onclick="grantRequest(this)">Grant</button>
            </div>
        <?php endforeach; ?>
    <?php else : ?>
    ...
    <?php endif; ?>
                                    ...
                                    <div id="receivedTabContent" class="tab-content p-4 hidden">
                                        <?php if (!empty($receivedMessages)) : ?>
                                            <?php foreach ($receivedMessages as $receivedMessage) : ?>
                                                <div class="border-t border-b border-gray-200 p-2 rounded received-item" style="width:300px;">
                                                    <!-- Your content for the "Received" tab goes here -->
                                                    <div class="text-sm font-medium">
                                                        <!-- Display relevant information for the "Received" tab -->
                                                    </div>
                                                    <div class="text-sm font-medium mb-1.5">
                                                        <!-- Display relevant information for the "Received" tab -->
                                                    </div>
                                                    ...
                                                    <button id="viewButton" class="px-4 py-2 cursor-pointer" onclick="viewReceived()">View</button>
                                                </div>
                                            <?php endforeach; ?>
                                        <?php else : ?>
                                    ...
                                    <script>
                                        function viewReceived() {
                                            // Your logic for handling the "View" action for received items
                                        }
                                    </script>
                                </div>
                                ...
                                <script>
                                    ...
                                    function showTab(tabContentId, tabIdToShow, tabIdToHide1, tabIdToHide2) {
                                        document.getElementById(tabContentId).style.display = 'block';
                                        document.getElementById(tabIdToShow).classList.add('bg-blue-300');
                                        document.getElementById(tabIdToShow).classList.remove('bg-white');

                                        var tabsToHide = [tabIdToHide1, tabIdToHide2];
                                        tabsToHide.forEach(function(tabId) {
                                            document.getElementById(tabId).classList.remove('bg-blue-300');
                                            document.getElementById(tabId).classList.add('bg-white');
                                            document.getElementById(tabId + 'Content').style.display = 'none';
                                        });
                                    }

                                    function grantRequest(button) {
                                        var selectedRequest = document.querySelector('#requestsTabContent .selected');
                                        if (selectedRequest) {
                                            document.getElementById('notificationTabContent').appendChild(selectedRequest);
                                            selectedRequest.classList.remove('selected');
                                            button.innerHTML = 'Receive';
                                            button.onclick = function() {
                                                receiveItem(selectedRequest);
                                            };
                                            showTab('notificationTabContent', 'notificationTab', 'requestsTab', 'receivedTab');
                                        }
                                    }

                                    function receiveItem(item) {
                                        var receivedTabContent = document.getElementById('receivedTabContent');
                                        receivedTabContent.appendChild(item);
                                        showTab('receivedTabContent', 'receivedTab', 'requestsTab', 'notificationTab');
                                    }
                                </script>
                            </div>
